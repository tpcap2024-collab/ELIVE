...
function validateAppsScriptResponse(
  ...
  if (
    ...
    throw new Error(
      `Google Apps Script ${action} returned no data.`
    );
  }
  ...
  if (
    ...
    throw new Error(
      String(
        data.error ||
        `${action} did not return success status.`
      )
    );
  }
}
...
async function requestAppsScriptGet(
  ...
  for (
    ...
    try {
      console.log(
        `Calling Apps Script GET ${action}, attempt ${attempt}`
      );
      ...
      console.error(
        `Apps Script GET ${action} connection error:`,
        {
          attempt,
...
function validatePlanPeriodRequest(body) {
  ...
  if (
    ...
    throw new Error(
      'Uploaded file has no valid Plan rows.'
    );
  }
...
app.listen(
  ...
  () => {
    console.log(
      `ELIVE API version 7 is running on port ${PORT}`
    );
  }
);
