...
export async function fetchTrucksFromSheets(): Promise<Truck[]> {
  ...
  // Use the local server proxy to avoid CORS issues
  const res = await fetch(`/api/trucks?url=${encodeURIComponent(APPS_SCRIPT_URL)}`);
...
export async function updateTruckInSheets(truckId: string, updates: Partial<Truck>, currentTruck: Truck): Promise<void> {
  ...
  const res = await fetch(`/api/trucks/update?url=${encodeURIComponent(APPS_SCRIPT_URL)}`, {
    method: 'POST',
    headers: {
  ...
  if (!res.ok) {
    throw new Error('Failed to update Google Sheet via Server Proxy');
  }
}
